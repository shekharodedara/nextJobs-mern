...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    const fetchChatUsers = async () => {
      try {
        ...
        setChatUsers(res);
        if (res?.length > 0) setSelectedUserId(res[0]._id);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await getConversation(currentUser._id, selectedUserId);
  ...
  const selectedUser = chatUsers ? chatUsers.find((u) => u._id === selectedUserId) : "null";
  const bgColor = selectedUser?.role === "jobSeeker" ? "bg-yellow-50" : "bg-pink-50";
